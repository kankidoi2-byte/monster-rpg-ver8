...
- 最新main追従後の `npm run check`（postcheck含む）、`git diff --check`、世界地図報酬比較が成功。画像128件・警告0、保存移行、320/360/390/430px静的配置、専用検査の保持を確認。配信script順序・既存ID・保存処理・世界地図完成画像は最新mainと同一。
