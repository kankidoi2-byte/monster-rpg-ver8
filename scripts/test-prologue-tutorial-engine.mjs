...
const chapterEnding=evaluate("normalizeTutorialStep({id:'chapter_end',chapterBreak:true,nextStepId:'next_chapter'},0)");
assert.equal(chapterEnding.chapterBreak,true,'episode endings must preserve their chapter break through normalization');
assert.equal(evaluate("normalizeTutorialStep({id:'dialogue'},0).chapterBreak"),false,'ordinary dialogue must not become an episode ending');
