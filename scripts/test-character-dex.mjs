...
const characterIds = Array.from(records.filter(record => record.entityKind === 'character'), record => record.id);

assert.deepEqual(characterIds, ['elna_beginner','elna_middle','elna_advanced','elna_water','elna_kaen']);
assert.deepEqual(Array.from(records.filter(record => record.entityKind === 'character'), record => record.characterNo), [1,2,3,4,5]);
...
assert(records.filter(record => record.id.startsWith('stella_') || record.id.startsWith('lumina_')).every(record => record.entityKind === 'monster'));
