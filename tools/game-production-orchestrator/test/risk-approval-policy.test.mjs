...
assert.equal(progress.current_cycle, 3);
assert.equal(progress.current_phase, 5);
assert.equal(progress.next_cycle, 4);
...
assert.equal(progress.latest_trial.id, 'kokoro-link-one-second-layout-20260903');
...
assert.equal(roadmap.includes('Cycle 4 / Phase 1'), true);
