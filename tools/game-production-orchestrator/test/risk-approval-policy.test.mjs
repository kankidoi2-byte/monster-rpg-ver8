...
assert.equal(progress.current_cycle, 5);
assert.equal(progress.current_phase, 5);
assert.equal(progress.next_cycle, 6);
...
assert.equal(progress.latest_trial.id, 'pr59-android-save-migration-verification-20260904');
...
assert.equal(roadmap.includes('Cycle 6 / Phase 1'), true);
