...
function multiStatusHtml(entry) { return statusHtml(entry.status,entry.poisonTurns,entry.paralysisTurns,entry.confusionTurns,entry.sleepTurns,entry.flareCharge,entry.aquaShield)+enemyKokoroLinkStatusHtml(entry.id); }
function multiEnemyKokoroLinkSpeed(entry){return enemyKokoroLinkSpeed(entry.mon,entry.id);}
...
function handleMultiEnemyCard(targetId){
  ...
  if(pendingKokoroLinkStatusSourceUid&&entry.alive){selectKokoroLinkStatusTarget(targetId);return;}
...
async function performMultiAttack(actor,target,move) {
  ...
  if((defenderIsPlayer?pHp:targetEntry.hp)>0){
    if(effect==='poison'){
      ...
      if(defenderIsPlayer){pStatus='poison';pPoisonTurns=BATTLE_STATUS_EFFECTS.poison.duration;}else{removeKokoroLinkEnemyEffectCategory(targetEntry.id,'poison');targetEntry.status='poison';targetEntry.poisonTurns=BATTLE_STATUS_EFFECTS.poison.duration;targetEntry.poisonSourceIsPlayer=actorIsPlayer;}
      msg+='<br>☠️ 毒状態になった！';
      }
    }
...
function winMultiBattle(){
  if(battleRewardGranted)return;battleRewardGranted=true;if(typeof resetKokoroLinkBattleState==='function')resetKokoroLinkBattleState();completeBattleTurn();multiBattle.finished=true;busy=true;if(typeof renderBattleInputState==='function')renderBattleInputState();
...
async function useMultiBattleContractScroll(itemId){
  ...
  if(ok){addInstance(entry.mon.id);if(typeof grantContractorContractSuccess==='function')grantContractorContractSuccess(entry.mon.id);}saveGame();show('battle');busy=true;if(typeof renderBattleInputState==='function')renderBattleInputState();await playContractAnimation({monsterName:entry.mon.name,stage:animationStage});
  if(ok){appendMultiLog(`🤝 ${it.name}を使い、${entry.mon.name}との契約に成功した！`);}else{appendMultiLog(`📜 ${it.name}を使ったが、${entry.mon.name}との契約には失敗した……`);}busy=false;if(typeof renderBattleInputState==='function')renderBattleInputState();updateItems();renderParty();renderDex();show('battle');renderMultiContractPanel();
}
