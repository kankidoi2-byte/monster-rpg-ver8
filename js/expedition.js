...
function expeditionEmptyMemberGuide(){
  const members=(save.instances||[]).filter(ins=>by(ins.id));
  if(!members.length)return '<p>派遣する仲間がいません。ストーリーや戦闘後の契約で仲間を増やしましょう。</p>';
  const partyMembers=members.filter(ins=>(save.party||[]).includes(ins.uid)&&!isInstanceOnExpedition(ins.uid));
  if(partyMembers.length>1)return '<p>派遣できる控えの仲間がいません。戦闘用の仲間を1体以上残して、編成から外すと派遣できます。</p><button type="button" onclick="show(\'partySet\')">パーティー編成を開く ›</button>';
  if(partyMembers.length===1)return '<p>戦闘用の仲間を1体残しましょう。戦闘後の契約で仲間を増やすか、派遣中の仲間の帰還・報酬受取を待ってください。</p>';
  return '<p>仲間は遠征中です。帰還済みなら報酬を受け取ってください。戦闘に出せる仲間がいない場合は、途中帰還で編成に戻せます。</p>';
}
function renderExpedition(){
  ...
  root.innerHTML='<p class="small">遠征は時間経過ではなく、バトルに勝つと進みます。派遣中の仲間は戦闘に編成できません。</p>'+`<div class="expedition-status"><div><span>ACTIVE SLOTS</span><strong>${active.length}/${unlocked}</strong></div><p>完了 ${save.expeditions.completedCount}回・${unlocked<2?'2枠目まであと'+(5-save.expeditions.completedCount):unlocked<3?'3枠目まであと'+(15-save.expeditions.completedCount):'全枠解放済み'}</p></div>`+
