...
function claimExpedition(id){const index=save.expeditions.active.findIndex(x=>x.id===id),entry=save.expeditions.active[index];if(!entry||entry.status!=='complete'||!entry.reward)return;grantExpeditionReward(entry,entry.reward);save.expeditions.active.splice(index,1);save.expeditions.completedCount++;saveGame();renderExpedition();renderParty();if(typeof renderPartySetup==='function')renderPartySetup();setTimeout(processNextEvolution,250);}
...
function renderExpedition(){
  ...
  root.innerHTML=`<div class="panel expedition-status"><h2>遠征枠 ${active.length}/${unlocked}</h2><p>完了回数 ${save.expeditions.completedCount}回 / ${unlocked<2?'2枠目まであと'+(5-save.expeditions.completedCount):unlocked<3?'3枠目まであと'+(15-save.expeditions.completedCount):'全枠解放済み'}</p></div>`+
    `<div class="expedition-active-grid">${Array.from({length:unlocked},(_,i)=>{const entry=active[i];if(!entry)return `<div class="panel"><h3>遠征枠${i+1}</h3><p class="small">空き</p></div>`;const map=MAPS.find(x=>x.id===entry.mapId),distance=EXPEDITION_DISTANCES[entry.distanceId],names=entry.memberUids.map(u=>by(getInstance(u)?.id)?.name||'不明').join('・');return `<div class="panel"><h3>${map?.name||entry.mapId}・${distance?.label||''}</h3><p>${names}</p><p>適性 ${entry.suitability.grade}（${entry.suitability.total}点）</p><p>進捗 ${entry.progress}/${entry.requiredWins}勝</p><p>${entry.status==='complete'?expeditionRewardText(entry.reward):'遠征進行中'}</p>${entry.status==='complete'?`<button onclick="claimExpedition('${entry.id}')">報酬を受け取る</button>`:`<button class="secondary-button" onclick="recallExpedition('${entry.id}')">途中帰還</button>`}</div>`;}).join('')}</div>`+
