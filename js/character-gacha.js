...
function rollCharacterGacha(count){
  ...
  target.innerHTML=`<h2>${result.count}体が仲間になりました</h2>${saved?'':'<p>保存に失敗しました。セーブ管理からデータを書き出してください。</p>'}<div class="character-gacha-grid">${result.entries.map(({unit,isNew,instance})=>`<article class="character-gacha-card">${vis(unit)}<strong>${unit.name}</strong><small>${unit.rarity} / Lv.1</small><span>${isNew?'NEW・図鑑登録':'同じ形態の別個体を獲得'}${instance.locked?'・🔒 自動ロック':''}</span></article>`).join('')}</div><button onclick="show('partySet')">編成する</button>`;
